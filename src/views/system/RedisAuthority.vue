  ...
  <v-row>
    <v-col>
      <v-alert
                  v-if="!isMatch"
                  density="compact"
                  text="不建议直接对Redis数据库进行修改，如确有需求，请点击下方申请权限为当前账号添加数据库访问权限。"
                  title="警告"
                  type="warning"
                />
    </v-col>
        ...
        <v-card>
              <v-card-title class="d-flex justify-space-between align-center">
                <div class="text-h2 text-medium-emphasis ps-2">
                  <v-icon icon="mdi-database" :color="isMatch ? 'success' : 'warning'"></v-icon> {{isMatch ? '验证通过' : '权限警告'}}
                </div>
              </v-card-title>
